...
class TrainingInstance(C274):
    ...
    def preprocess_words(self, mode=''):  # Task_1 #
        ...
        if mode == "keep-symbols":
            Sym = nonalpha(self.numbers, lowered)
            fin = final(Sym)
        elif mode == "keep-stops":
            Sym = nonalpha(self.spechar, lowered)
            Num_Sym = nonalpha(self.numbers, Sym)
            fin = Num_Sym
...
class TrainingSet(C274):
    ...
    def preprocess(self, mode=''):  # Task_1 #
        for _, cuh in enumerate(self.get_instances()):
            cuh.preprocess_words(mode)
        return
    # Task_1 #
    ...
    def return_nfolds(self, num=3): # Task_3 #
        ...
        for i in range(num):
            TS = TrainingSet()
            countr = i
            while countr < len(self.inObjHash):
                TS.inObjHash += [copy.deepcopy(self.inObjHash[countr])]
                TS.inObjList += [copy.deepcopy(self.inObjList[countr])]
                countr += num
            n += [TS]
        return (n)
    ...
    def add_training_set(self, trainingset):
        ...
        for i in trainingset.inObjHash:
            self.inObjHash += [copy.deepcopy(i)]
            self.inObjList += [copy.deepcopy(i)]
        return
    # Task_3 #
...
class ClassifyByTopN(ClassifyByTarget):
    ...
    def target_top_n(self, tset, num=5, label=''):
        dict = {}
        for e in (tset.inObjHash):
            if e.get_label() == label:
                for word in e.get_words(): 
                    if word not in dict: 
                        dict[word] = 0 
                    dict[word] += 1
        new = []
