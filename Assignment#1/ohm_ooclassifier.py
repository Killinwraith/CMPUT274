...
class TrainingInstance(C274):
    ...
    def process_input_line(
        ...
        return (self)
    ###-Task 1-###
    #splits the words by the characters as to remove them
    def char_get_gone(self, string): return ''.join([str(x) for x in string if not x in "@#!-$%&'()*+,./:;<=>?_[]^`{|}~"])
    def num_get_gone(self, string):  return ''.join([str(x) for x in string if not x in "0123456789"]) if not string.isnumeric() else string
 
    #cleans the words in a list
    def line_formation(self, lit, exep):
        out = []
        for word in lit: 
            word.lower()
            if exep != 'keep-symbols': word = self.char_get_gone(word)
            if exep != 'keep-digits': word = self.num_get_gone(word)
            if not((exep != 'keep-stops') == True == (word.lower() in self.Stop_Words)) : out += [word.lower()]
        return out

    def preprocess_words(self,mode):
        #preprocess's inst["words"] with whihc ever mode was called
        self.inst["words"] = self.line_formation(self.inst["words"],mode)
    ###-Task 1-###
...
class TrainingSet(C274):
    ...
    def preprocess(self, mode=''):
        #runs preprocess_words for every list in instances
        for training_instance in self.get_instances(): 
            training_instance.preprocess_words(mode)
    ###-Task 1-###
...
class ClassifyByTopN(ClassifyByTarget):
    def __init__(self, lw=[]):
        self.type = str(self.__class__)
        super().__init__(lw)  # Calls the superclass method
    # Requirement 2
    def target_top_n(self, tset, num=5, label=''):
        dictionary = {}
        for elem in (tset.inObjHash):
            if elem.get_label() == label:
                for word in elem.get_words(): 
                    if word not in dictionary: dictionary[word] = 0 
                    dictionary[word] += 1
        new_target_words = []
        #adds the num most common word in the dictionary to target words
        while len(new_target_words) < num:
            max_freq = max(dictionary.values())
            for word in dictionary:
                if dictionary[word] == max_freq:
                    new_target_words += [word]
                    dictionary[word] = 0
        #removes label if one
        if label in new_target_words: new_target_words.remove(label)
